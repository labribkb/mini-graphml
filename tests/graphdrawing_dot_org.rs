...
#[test]
fn at_t() {
    handle_dataset("http://www.graphdrawing.org/data/north-graphml.tgz", Path::new("data/north"));
}

#[test]
fn random() {
    handle_dataset("http://www.graphdrawing.org/data/random-dag-graphml.tgz", Path::new("data/random-dag"));
}
...
fn handle_dataset(url: &str, path: &Path) {
    ...
    for fname in glob::glob(path.join("rome").join("*.graphml").to_str().unwrap()).unwrap() {
        ...
        let g1 = GraphMLNoData::load(fname).unwrap_or_else(|_| panic!("Unable to load {str_fname}"));
